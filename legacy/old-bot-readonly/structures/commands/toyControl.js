...
const LovenseConnect = require('./LovenseConnect');
const { LovenseConnect_send, LovenseConnect_getQrCode, LovenseConnect_GetConnectedToys, LovenseConnect_logout} = require('./LovenseConnect');
const { getDataFromDB } = require('../sql/Pool.js');
const { ChartJSNodeCanvas } = require('chartjs-node-canvas');
let config='';
require('dotenv').config();
const {DEV, CHART_LINK, CHAN_ID_GANGBANG, CHAN_ID_ORGY, EMOJI_COUNTDOWN_5} = process.env;

const Lovense_check_session_start = async (interaction, messageId ) => {
	const followUp = await interaction.reply({ content: `Starting Session... please wait...`, ephemeral: true, fetchReply: true });
	...
	if(session === undefined){
		...
		.addComponents(
			new ButtonBuilder()
				.setCustomId(`lovenseleave-${messageId}`)
		...
		//interaction.webhook.editMessage(followUp.id, {text: 'You are already controlled in another session, do you want to leave the other session?', components: [row], ephemeral: true}).catch(err => console.error(`SEND already in session EMBED to`,err));
		await interaction.editReply({content: 'You are already controlled in another session, do you want to leave the other session?', components: [row], ephemeral: true}).catch(err => console.error(`Error: toycontrol 39`,err));
...
const Lovense_delete_member_session = async (interaction ) => {
	...
	if(session === undefined){
		await interaction.reply({content: `Only the owner can end a session and only members who get controlled can leave a session.`, ephemeral: true});
		setTimeout(() => {
			interaction.deleteReply()
		}, 5000) 
	} else {
		...
		if(sessionfound === undefined){
			...
			interaction.
			reply({content: `Session ended.`, ephemeral: true});
			setTimeout(() => {
				interaction.deleteReply()
			}, 5000) 
		} else {
			if(interaction.customId === 'lovenselogout'){
				interaction.reply({content: `You left the session.`, ephemeral: true});
				setTimeout(() => {
					interaction.deleteReply()
				}, 5000) 
			} else {
				interaction.editReply({content: `You left the session.`, ephemeral: true});
				setTimeout(() => {
					interaction.deleteReply()
				}, 5000) 
			}
...
async function Lovense_join_session(interaction) {
	...
	Lovense_create_qr_embed(followUpMsg);
	const filter = i => i.customId === 'lovenselogingangbang';
	const collector = interaction.channel.createMessageComponentCollector({ filter });
	...
	*/
	let messageID = interaction.customId.substring(16, interaction.customId.length)
	let queryinsertinto2 = `INSERT INTO toycontrol_user (discord_message_id, discord_user_id) VALUES (${String(messageID)}, ${String(interaction.member.id)})`;
	await getDataFromDB(queryinsertinto2);
	const row = new ActionRowBuilder()
	.addComponents(
		new ButtonBuilder()
			.setCustomId('lovensestay')
			.setLabel('Please dismiss this message')
			.setStyle(ButtonStyle.Secondary)
	);
	await interaction.update({content: 'You joined the session', embeds: [], components: [], ephemeral: true}).catch(err => console.error(`Error: toycontrol 39`,err));

}

async function Lovense_create_qr_embed(interaction) {
		...
		if(img.result === true){
			...
			if(interaction.customId.substring(0, 13) === 'lovenseleave-' ){
				customId = interaction.customId.substring(13, interaction.customId.length);
			}
			if(customId == "startGangbang"){
				row = new ActionRowBuilder()
				.addComponents(
					new ButtonBuilder()
						.setCustomId('lovenselogingangbang')
						.setLabel(`✅ I'm ready and followed the above steps`)
						.setStyle(ButtonStyle.Secondary)
				);
			} else if(customId == "startOrgy"){
				row = new ActionRowBuilder()
				.addComponents(
					new ButtonBuilder()
						.setCustomId('lovenseloginorgy')
						.setLabel(`✅ I'm ready and followed the above steps`)
						.setStyle(ButtonStyle.Secondary)
				);
			} else if(customId == "tipPlayPanelButtonStart"){
				row = new ActionRowBuilder()
				.addComponents(
					new ButtonBuilder()
						.setCustomId('lovenselogintip')
						.setLabel(`✅ I'm ready and followed the above steps`)
						.setStyle(ButtonStyle.Secondary)
				);
			} else if(customId.substring(0, 12) === 'lovensejoin-' ){
				let messageID = interaction.customId.substring(12, interaction.customId.length)
				row = new ActionRowBuilder()
				.addComponents(
					new ButtonBuilder()
						.setCustomId(`lovensejoinorgy-${messageID}`)
						.setLabel(`✅ I'm ready and followed the above steps`)
						.setStyle(ButtonStyle.Secondary)
				);
			} else {
				...
				.addComponents(
					new ButtonBuilder()
						.setCustomId(`lovensejoinorgy-${customId}`)
						.setLabel(`✅ I'm ready and followed the above steps`)
						.setStyle(ButtonStyle.Secondary)
				);
			}
			...
			await interaction.editReply({content: `Scan QR code and click the button to start/join the session`, embeds: [qrcodeMessage], components: [row], ephemeral: true}).catch(err => console.error(`SEND lovense login EMBED`,err));
...
async function Lovense_Start_session(interaction) {
	...
	let toy=[];
	if(DEV == 'true'){
		toy=['TestToy','TestToy2'];
	}else{
		toy = await LovenseConnect_GetConnectedToys(interaction.user.id);
	}
	...
	if(toy[0] !="appoffline" && toy[0] !="toyoffline"){				
		...
		if(subcommand == "tip"){
			...
			lovenseEmbed.setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL({dynamic: true})});
			...
			if (subcommand == "tiporgy") {
				...
				.addComponents(
					...
					new ButtonBuilder()
						.setCustomId(`lovensejointip-${interaction.messageID}`)
			...
			if (privacy == "private") {
				...
				sessionMsg = await channel.send({embeds: [lovenseEmbed], components: [row]})
				
				//await channel.send({embeds: [lovenseEmbed], components: [row]})

				//change message reply to link to post, add permissions and such remove old emed message?
				...
				let queryinsertinto = `INSERT INTO toycontrol (discord_guild_id, discord_channel_id, discord_message_id, discord_user_ids, session_active, session_mode, session_type, toy_speed) VALUES (${String(interaction.guildId)}, ${String(channel.id)}, ${String(sessionMsg.id)}, ${String(interaction.member.id)}, 1, '${subcommand}', '${subcommand}', '${defaultspeedjson}')`;
				...
				*/
				...
				await channel.send({embeds: [lovenseEmbedThread]}).then(
					//sentMessage => interaction.client.lovense.panel = sentMessage
				);
				console.log(`Created thread: ${thread.name}`);
				await interaction.editReply({content: `Session created successfully https://discord.com/channels/${sessionMsg.guildId}/${sessionMsg.channelId}/${sessionMsg.id}`, embeds: [], components: [], ephemeral: true}).catch(err => console.error(`Error: toycontrol 422`,err));
			...
			*/
			...
			const lovenseEmbed = new EmbedBuilder();
			lovenseEmbed.setTitle(`Toycontrol ${subcommand} session`);
			lovenseEmbed.setDescription(message);
			...
			if (subcommand == "orgy") {
				...
				.addComponents(
					new ButtonBuilder()
						...
						.setStyle(ButtonStyle.Danger),
					new ButtonBuilder()
						.setCustomId(`lovensejoin-${interaction.messageID}`)
						.setLabel('➕ Get controlled')
						.setStyle(ButtonStyle.Success)
				);
			}else{
				...
				if (subcommand == "orgy") {
					...
					.addComponents(
						new ButtonBuilder()
							...
							.setStyle(ButtonStyle.Danger),
						new ButtonBuilder()
							.setCustomId(`lovensejoin-${sessionMsg.id}`)
							.setLabel('➕ Get controlled')
							.setStyle(ButtonStyle.Success)
					);
					await sessionMsg.edit({components: [row]})
				}
			...
			//await sessionMsg.react('♒').catch(err => { console.error('5 one of the emojis failed to react.', err)} );
			await sessionMsg.react(EMOJI_COUNTDOWN_5).catch(err => { console.error('5 one of the emojis failed to react.', err)} );
			...
			*/
			...
			/*if(!config.discord_user_ids.includes(i.user.id)){
				...
				if(toy[0] =="appoffline"){
					//await i.reply();
					await interaction.followUp({content: `<@${interaction.user.id}> The Lovense Remote App is not running or the session is expired. Please start the Lovense connect app, connect your toy and try again.\n(Sometimes you need to end the current session and re-scan the QR code or the Lovense Connect app is buggy and needs a restart)`, ephemeral: true}).then();
				}else{
					await interaction.followUp({content: `<@${interaction.user.id}> There is no toy connected to Lovense Connect, make sure that at least one toy is connected in the app and ✅ react again.\n(Sometimes the app is bugged and needs a restart)`, ephemeral: true}).then();
				}
			//}
...
async function Lovense_TipPlay(interaction, config, sessionMsg, collected) {
	// start a session loop to listen for new tips in the database send them to the toy in the correct order
	let count = 0;
	let stop = false;
	while (stop === false) {
		try{
			...
			if(query_sessions_toycontrol.length === 0){
				...
				//check if there is a new tip
				let query_tips = `SELECT * FROM tips WHERE discord_message_id=${sessionMsg.id} AND tip_id > ${count}`;
				let query_sessions_tips = await getDataFromDB(query_tips);
				if(query_sessions_tips.length > 0){
...
async function Lovense_ReactionVote(config, sessionMsg) {
	...
	try{
		...
		if(query_sessions_toycontrol.length === 0){
			...
			//check if all user are in the map, none is missing, not too many and check if toys are still connected once per miunte 
			for (let i = 0 ; i< query_sessions_toycontrol_user.length; ++i){
				if(count%12 == 0 || (count%12 != 0 && !user.has(query_sessions_toycontrol_user[i].discord_user_id))){
					let toy=[];
					if(DEV == 'true'){
						toy=['TestToy','TestToy2'];
						if(!user.has(query_sessions_toycontrol_user[i].discord_user_id)){
							//add new user with toys
							user.set(query_sessions_toycontrol_user[i].discord_user_id, toy);
						}
					}else{
						user.delete(query_sessions_toycontrol_user[i].discord_user_id)
						toy = await LovenseConnect_GetConnectedToys(query_sessions_toycontrol_user[i].discord_user_id);
						if(toy[0] !="appoffline" && toy[0] !="toyoffline"){
							//delete from session or write note	
						} else {	
							user.set(query_sessions_toycontrol_user[i].discord_user_id, toy);
						}					
					}
			...
			for (let key in user) {
				if (!query_sessions_toycontrol_user.includes(user[key])) {
					delete user[key];
					//remove from session
				}
			}
			...
			let highestEmojiSpeed = sessionMsg.reactions.cache
				.map(r => {return {name: r.emoji.name, count: r.count-1}})
				.reverse()
				.sort((a, b) => b.count - a.count)
				.filter(r => ['0️⃣', '1️⃣', '2️⃣', '3️⃣', '4️⃣', '5️⃣']
				.includes(r.name))[0]
			...
			if(votedspeed === undefined ){
				...
				await sessionMsg.react('4️⃣').catch(err => { console.error('4 one of the emojis failed to react.', err)} );
				await sessionMsg.react('5️⃣').catch(err => { console.error('5 one of the emojis failed to react.', err)} );
				await sessionMsg.react(EMOJI_COUNTDOWN_5);
				/*
				await sessionMsg.react('⬆').catch(err => { console.error('⬆ one of the emojis failed to react.', err)} );
				await sessionMsg.react('⬇').catch(err => { console.error('⬇ one of the emojis failed to react.', err)} );
				await sessionMsg.react('⏸').catch(err => { console.error('⏸ one of the emojis failed to react.', err)} );
			...
			if(votes.length < 7 ){
				...
				*/
				...
				await sessionMsg.reactions.cache.get(EMOJI_COUNTDOWN_5).remove();
				await sessionMsg.react(EMOJI_COUNTDOWN_5);
			}
			...
			//const reactionEmoji = sessionMsg.guild.emojis.cache.get(EMOJI_COUNTDOWN_5); // Replace with your emoji id



			//embed.setAuthor({ name: '⬅️ Next reaction count', iconURL: 'https://cdn.discordapp.com/attachments/795952030228414474/1084945594407206952/countdown-no-loop.gif?'+Math.random()*Math.random()});

			embed.setThumbnail(`https://cdn.discordapp.com/attachments/795952030228414474/1084945594407206952/countdown-no-loop.gif?`+Math.random()*Math.random());
			embed.setFooter({ text: '⬅️Next vote | Session started➡️', iconURL: `https://cdn.discordapp.com/attachments/795952030228414474/1084945594407206952/countdown-no-loop.gif?`+Math.random()*Math.random() });
			//embed.spliceFields(0, 1, {name: 'User / Toy', value: user.map(u => `<@${u}>`).join('\n')});
			...
			embed.spliceFields(0, 1, {name: 'User / Toy', value: text});
			...
			if(count%12 == 0){
				...
				sessionMsg.react(EMOJI_COUNTDOWN_5);
...
module.exports.Lovense_create_qr_embed = Lovense_create_qr_embed
