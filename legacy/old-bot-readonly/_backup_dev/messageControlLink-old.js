...
const { ActionRowBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder} = require("discord.js"); // This is to send the image via discord.
const roleRegex = /^Gender: (\S+)/;
const roleRegexOrientation = /^Sexual Orientation: (\S+)/;
/* eslint-disable no-useless-escape */
const text_regex = /(.*?)(?=\[)/g;
const toys_regex = /(?<=\[)(.*?)(?=\])/g;
const time_regex = /\[(([+-]?(?=\.\d|\d)(?:\d+)?(?:\.?\d*))(?:[eE]([+-]?\d+))?(:([+-]?(?=\.\d|\d)(?:\d+)?(?:\.?\d*))(?:[eE]([+-]?\d+))?)+)\]/g;
...
/* deactivated, because lovense doesn't let us get toys of links anymore
//https://c.lovense.com/c/
	// we process the first link.
	links_new = links_new.shift();
	const words = links_new.split('/c/');
	let links = '';
	if(words.length == 2){
	...
	switch (returnMessage.result) {
		case true: {
			...
			if (toysON > 0) {
				...
				embed.addFields({name: 'Session duration', timeLeft, inline: true});
...
//embed.setThumbnail(`https://cdn.discordapp.com/emojis/${emojis[toyNames[i]]}.png`); // last
embed.addFields({name: 'Toys', value: `${toys}`, inline: true})
...
//embed.addFields({name: 'Control requests', '0', true);
	...
	if (shallSend) {
		...
		const sDesc = JSON.stringify(text);
