...
| Control link via DM | `handlers/dmMessage.js`, `handlers/messageControlLink.js` (`button_start_control_link` flow) | Migrated (redesigned) | `apps/bot/src/features/control-link/control-link-dm.*` | DMing the bot a supported link replies with a "Start" button, then an anonymous/reveal choice, then an optional message step (modal input), then posts into the existing configured control-link channel (`CHAN_ID_TOY_LINK`) via the same `postRaffleEmbed`/raffle join-pick-winner flow used for in-channel links. **Dropped vs legacy:** the public-vs-verified-only channel split (only one control-link channel exists in the new bot; membership in that channel's guild is still required) and the never-shipped preference-matching step. `Raffle.anonymous` / `Raffle.message` are new nullable columns. New `apps/bot/src/modals/` registry added (first feature to need modal submissions). |
| Control link via slash command | (new) | Added | `apps/bot/src/features/control-link/control-link.commands.ts` | `/control-link-raffle` — opens a modal to collect the link (rather than a string option) so the raw URL never appears in the channel's public "used /command" line, then joins the same anonymous/reveal → optional message → post-to-channel wizard used by the DM flow (shared customId prefix `control-link-raffle:`, shared button/modal handlers). |
