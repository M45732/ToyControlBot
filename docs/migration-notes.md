...
| 2026-06-20 | Enable the privileged `GuildMembers` intent | The subscriptions feature must reliably read a single thread's membership before granting/revoking fanclub access; without it `/subscribe` can't tell a pre-existing member from a non-member and aborts rather than risk a wrong add/kick. **Must be toggled on in the Discord Developer Portal** ("Server Members Intent") or the bot fails to log in |
...
| Control link via slash command | N/A (new) | `/control-link-raffle` opens a modal for the link instead of taking it as a command option | A plain string option's value is shown publicly in the channel's "used /command" line even when the bot's own reply is ephemeral, which would leak the link exactly like posting it as a normal message — modals are the only interaction surface where free text stays private to the invoking user |
